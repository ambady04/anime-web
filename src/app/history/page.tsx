...
export default function HistoryPage() {
  ...
  const handleClearHistory = () => {
    if (window.confirm('Are you sure you want to clear your full watch history?')) {
  ...
  return (
    <div className="max-w-[95rem] mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-20 animate-fade-in">
      {/* Header Info */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10 pb-6 border-b border-glass-border select-none">
        <div>
          <h1 className="text-2xl sm:text-3.5xl font-black text-foreground flex items-center space-x-2">
            ...
            <span className="uppercase tracking-wider">Watch History</span>
          </h1>
          <p className="text-xs sm:text-sm text-foreground/50 font-medium mt-1">Keep track of what you started watching.</p>
        </div>
      ...
      {history.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {history.map((item) => {
            ...
            return (
              ...
              >
                {/* Image side */}
                <div className="relative w-1/3 aspect-[16/9] md:aspect-auto flex-shrink-0 bg-foreground/5 overflow-hidden">
                ...
                <div className="p-4 flex flex-col justify-between flex-grow">
                  <div>
                    <div className="flex items-start justify-between">
                      ...
                      {/* Remove item button */}
                      <button
                        onClick={(e) => handleRemoveItem(e, item.detailPath)}
                  ...
                  <div className="mt-4 space-y-1.5">
                    ...
                    <div className="w-full h-1 bg-foreground/10 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-primary to-primary-light shadow-[0_0_6px_var(--primary-glow)]"
        ...
        <div className="text-center py-16 bg-glass-card rounded-3xl border border-glass-border p-8 max-w-md mx-auto shadow-sm select-none">
          ...
          <h3 className="text-sm font-black text-foreground uppercase tracking-wider mb-1">No History Logs</h3>
          <p className="text-xs text-foreground/50 font-medium max-w-xs mx-auto mb-6">
            You haven&apos;t started watching any videos yet. Start streaming to track your progress!
          </p>
          <Link
            href="/"
