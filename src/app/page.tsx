...
export default async function HomePage() {
  ...
  // Extract sections
  const bannerModule = homeData?.operatingList?.find((m) => m.type === 'BANNER');
  const banners = bannerModule?.banner?.items || [];

  const filterModule = homeData?.operatingList?.find((m) => m.type === 'FILTER');
  const categories = filterModule?.filters || [];

  const shelves = homeData?.operatingList?.filter(
    (m) => m.type === 'SUBJECTS_MOVIE' && m.subjects && m.subjects.length > 0
  ) || [];
  ...
  return (
    <div className="pb-16 relative">
      ...
      {categories.length > 0 && (
        <div className="max-w-[95rem] mx-auto px-4 sm:px-6 lg:px-8 my-10 relative z-20">
          <div className="flex items-center space-x-2 mb-6 select-none">
            <Layers className="w-4 h-4 text-primary" />
            <h2 className="text-xs font-black uppercase tracking-wider text-foreground/45">Browse by Genre</h2>
          </div>
      ...
      {shelves.length > 0 ? (
        ...
        <div className="max-w-md mx-auto my-24 p-8 rounded-3xl glass-panel border border-glass-border text-center shadow-2xl relative z-20 select-none">
          ...
          <h2 className="text-lg font-black text-foreground uppercase tracking-wider mb-2">Service Temporarily Offline</h2>
          <p className="text-xs text-foreground/70 mb-6 font-medium">
            We are experiencing problems fetching catalogs from the live media backend API.
          </p>
        ...
        <div className="max-w-[95rem] mx-auto px-4 py-20 text-center text-foreground/50 font-bold uppercase tracking-wider text-xs animate-pulse select-none">
