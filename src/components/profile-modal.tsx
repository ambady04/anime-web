...
export default function ProfileModal({ isOpen, onClose }: ProfileModalProps) {
    ...
    const [activeRightTab, setActiveRightTab] = useState<"manager" | "forceOps" | "preferences">("manager");
    ...
    const [managerTab, setManagerTab] = useState<"watchlist" | "history">("watchlist");
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setPrefSubtitleSize(localStorage.getItem("player-subtitle-size") || "22px");
            setPrefAutoplay(localStorage.getItem("player-autoplay") !== "false");
            setPrefAutoResume(localStorage.getItem("player-auto-resume") !== "false");
        } else {
            setCloudWatchlist([]);
            setCloudHistory([]);
        ...
        try {
            const backup: Record<string, any> = {
                ...
                settings: {
                    ...
                    subtitleSize: localStorage.getItem("player-subtitle-size") || "22px",
                    autoplay: localStorage.getItem("player-autoplay") || "true",
                    autoResume: localStorage.getItem("player-auto-resume") || "true",
                }
            };
    ...
    const handleImportBackup = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        fileReader.onload = async (event) => {
            try {
                ...
                if (!backup || typeof backup !== "object") throw new Error("Invalid format");

                // Restores watchlist
                if (Array.isArray(backup.watchlist)) {
                    localStorage.setItem("kixo_watchlist", JSON.stringify(backup.watchlist));
                }

                // Restores history
                if (Array.isArray(backup.history)) {
                    localStorage.setItem("kixo_history", JSON.stringify(backup.history));
                }
                ...
                if (backup.settings && typeof backup.settings === "object") {
                    if (backup.settings.theme) {
                        localStorage.setItem("kixo_theme", backup.settings.theme);
                        updatePrefTheme(backup.settings.theme);
                    }
                    if (backup.settings.subtitleSize) {
                        localStorage.setItem("player-subtitle-size", backup.settings.subtitleSize);
                        setPrefSubtitleSize(backup.settings.subtitleSize);
                    }
                    if (backup.settings.autoplay) {
                        localStorage.setItem("player-autoplay", backup.settings.autoplay);
                        setPrefAutoplay(backup.settings.autoplay === "true");
                    }
                    if (backup.settings.autoResume) {
                        localStorage.setItem("player-auto-resume", backup.settings.autoResume);
                        setPrefAutoResume(backup.settings.autoResume === "true");
                    }
                }

                setSyncMessage("Data successfully imported! Syncing with cloud...");
    ...
    const handleForcePush = async () => {
        ...
        try {
            ...
            localWatchlist.forEach((item) => {
                const docRef = doc(db, "users", uid, "watchlist", encodeURIComponent(item.detailPath));
                uploadBatch.set(docRef, item);
            });

            const localHistory = localStore.getHistory();
            localHistory.forEach((item) => {
                const docRef = doc(db, "users", uid, "history", encodeURIComponent(item.detailPath));
                uploadBatch.set(docRef, item);
            });
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key && key.startsWith("kixo_ep__")) {
                    ...
                    if (data) {
                        const docRef = doc(db, "users", uid, "watched_episodes", encodeURIComponent(cleanKey));
                        uploadBatch.set(docRef, { episodes: JSON.parse(data), updatedAt: Date.now() });
                    }
                }
            }
            ...
            setSyncMessage("Local state forced to Cloud database successfully!");
            setTimeout(() => setSyncMessage(""), 4000);
        } catch (err) {
    ...
    const handleForcePull = async () => {
        ...
        try {
            ...
            for (let i = localStorage.length - 1; i >= 0; i--) {
                const key = localStorage.key(i);
                if (key && key.startsWith("kixo_ep__")) localStorage.removeItem(key);
            }
            ...
            await fetchCloudData(uid);
            setSyncMessage("Cloud pulled successfully! Reloading to apply changes...");
            setTimeout(() => {
                setSyncMessage("");
                window.location.reload();
            }, 1500);
    ...
    const toggleSelectAll = () => {
        const currentList = managerTab === "watchlist" ? cloudWatchlist : cloudHistory;
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            if (managerTab === "watchlist") {
                pathsToDelete.forEach((path) => {
                    ...
                    for (let i = localStorage.length - 1; i >= 0; i--) {
                        ...
                        if (key && key.startsWith(`kixo_ep__${path}__s`)) {
                            ...
                            const cleanKey = key.replace("kixo_ep__", "");
                            const epDocRef = doc(db, "users", user.uid, "watched_episodes", encodeURIComponent(cleanKey));
                ...
                const updatedLocal = localStore.getWatchlist().filter((item) => !pathsToDelete.includes(item.detailPath));
                ...
                const updatedLocal = localStore.getHistory().filter((item) => !pathsToDelete.includes(item.detailPath));
                        ...
                        {showResetConfirm ? (
                            ...
                            <div className="space-y-6 flex-1 py-4">
                                <div className="flex flex-col items-center text-center space-y-3 select-none">
                                    ...
                                    <p className="text-xs text-foreground/60 font-medium max-w-xs leading-relaxed">
                                        This resets your local browser data. If you are logged in, your cloud sync backups will be preserved.
                                    </p>
                                </div>
                            ...
                            <div className="space-y-6 flex-1 py-4">
                                <div className="flex flex-col items-center text-center space-y-3 select-none">
                                    ...
                                    <p className="text-xs text-foreground/60 font-medium max-w-xs leading-relaxed">
                                        This completely deletes all data (Watchlist, History, Episodes) synced with your Google account. Your local device states will not be deleted.
                                    </p>
                                </div>
                                <div className="bg-white/5 border border-white/5 rounded-2xl p-4 space-y-3 text-xs max-w-md mx-auto">
                                    <div className="flex items-center justify-between text-white/80">
                                        <span className="font-semibold">Cloud Watchlist / Bookmarks</span>
                                        <span className="text-red-500 font-bold">Deleted Forever</span>
                                    </div>
                                    <div className="flex items-center justify-between text-white/80 border-t border-white/5 pt-3">
                                        <span className="font-semibold">Cloud Playback Progress</span>
                                        <span className="text-red-500 font-bold">Deleted Forever</span>
                                    </div>
                                </div>
                                <div className="flex gap-3 max-w-md mx-auto">
                            ...
                            <>
                                ...
                                <div className="w-full md:w-64 shrink-0 flex flex-col justify-between border-b md:border-b-0 md:border-r border-white/10 pb-6 md:pb-0 md:pr-6 overflow-y-auto pr-1">
                                    <div className="space-y-5">
                                        ...
                                        {!user ? (
                                            ...
                                            <div className="space-y-4 pt-2">
                                                <div className="flex items-center space-x-3 select-none">
                                                    ...
                                                    <div>
                                                        <h4 className="text-sm font-black text-white">Guest Account</h4>
                                                        <span className="text-[10px] text-foreground/45 block">Not signed in</span>
                                                    </div>
                                                </div>
                                                <button
                                        ...
                                        <div className="bg-white/5 border border-white/5 rounded-2xl p-3.5 space-y-3">
                                            <div>
                                                <span className="text-[10px] font-bold text-white uppercase tracking-wider block">Local Backups</span>
                                                <span className="text-[9px] text-foreground/45 block mt-0.5">Export or restore browser files</span>
                                            </div>
                                            <div className="flex gap-2">
                                                <button
                                                    onClick={handleExportBackup}
                                                    className="flex-1 flex items-center justify-center space-x-1 py-2 px-3 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/5 hover:border-white/10 text-white font-bold text-[10px] uppercase tracking-wider cursor-pointer"
                                                >
                                ...
                                <div className="flex-1 flex flex-col min-w-0 h-[380px] md:h-full overflow-hidden">
                                    ...
                                    <div className="flex space-x-2 border-b border-white/5 pb-2 shrink-0 select-none">
                                        <button
                                            onClick={() => setActiveRightTab("manager")}
                                        ...
                                        <button
                                            onClick={() => setActiveRightTab("forceOps")}
                                            className={`flex items-center space-x-1 px-3 py-1.5 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-colors cursor-pointer ${
                                        ...
                                        <button
                                            onClick={() => setActiveRightTab("preferences")}
                                            className={`flex items-center space-x-1 px-3 py-1.5 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-colors cursor-pointer ${
                                    ...
                                    <div className="flex-1 min-h-0 overflow-y-auto mt-4 pr-1 custom-scrollbar">
                                            ...
                                            <div className="space-y-4">
                                                {!user ? (
                                                    <div className="py-12 text-center text-xs text-foreground/40 font-medium">
                                                        Please sign in with Google to manage your cloud sync database.
                                                    </div>
                                                ) : (
                                                    <>
                                                        {/* Sub-tab selection */}
                                                        ...
                                                        <div className="flex items-center justify-between text-xxs text-foreground/60 px-1 select-none">
                                                            ...
                                                            >
                                                                {selectedItems.length === (managerTab === "watchlist" ? cloudWatchlist.length : cloudHistory.length) && (managerTab === "watchlist" ? cloudWatchlist.length : cloudHistory.length) > 0
                                                                    ? "Deselect All"
                                                                    : "Select All"}
                                                            </button>
                                                        ...
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto pr-1">
                                                            {managerTab === "watchlist" ? (
                                                                cloudWatchlist.length === 0 ? (
                                                                    ...
                                                                    cloudWatchlist.map((item) => (
                                                                        <div
                                                                            ...
                                                                            className="flex items-center space-x-2.5 bg-white/[0.02] border border-white/5 hover:bg-white/5 rounded-2xl p-2 cursor-pointer transition-all duration-200"
                                                                        >
                                                                ...
                                                                cloudHistory.length === 0 ? (
                                                                    <div className="col-span-full py-8 text-center text-xs text-foreground/40 font-medium">
                                                                        No history synced in the cloud.
                                                                    </div>
                                                                ) : (
                                        ...
                                        {activeRightTab === "forceOps" && (
                                            <div className="space-y-4">
                                                {!user ? (
                                                    <div className="py-12 text-center text-xs text-foreground/40 font-medium">
                                                        Please sign in with Google to run conflict resolution operations.
                                                    </div>
                                                ) : (
                                                    <div className="grid grid-cols-1 gap-3">
                                                        ...
                                                        <div className="bg-white/5 border border-white/5 p-4 rounded-2xl space-y-3">
                                                            <div className="flex items-start space-x-3">
                                                                ...
                                                                <div>
                                                                    <h4 className="text-xs font-black text-white uppercase tracking-wider">
                                                                        Force Local to Cloud (Overwrite Cloud)
                                                                    </h4>
                                                                    <p className="text-[10px] text-foreground/60 leading-relaxed mt-1">
                                                                        Replaces all synced cloud datasets in Firestore with the current local browser data (history, watchlist, episode progression). Use this if your local states are correct and you want to reset your cloud backup.
                                                                    </p>
                                                                </div>
                                                            </div>
                                                            <div className="flex justify-end pt-1 select-none">
                                                                ...
                                                                >
                                                                    ...
                                                                    <span>{isForcePushing ? "Pushing..." : "Overwrite Cloud Backup"}</span>
                                                                </button>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="bg-white/5 border border-white/5 p-4 rounded-2xl space-y-3">
                                                            <div className="flex items-start space-x-3">
                                                                ...
                                                                <div>
                                                                    <h4 className="text-xs font-black text-white uppercase tracking-wider">
                                                                        Force Cloud to Local (Overwrite Local)
                                                                    </h4>
                                                                    <p className="text-[10px] text-foreground/60 leading-relaxed mt-1">
                                                                        Replaces all local browser cache states with the remote cloud database copy. This wipes all bookmarks and progress on this local device and overrides them with your cloud account. (Triggers a quick page reload).
                                                                    </p>
                                                                </div>
                                                            </div>
                                                            <div className="flex justify-end pt-1 select-none">
                                                                ...
                                                                >
                                                                    ...
                                                                    <span>{isForcePulling ? "Pulling..." : "Overwrite Local Device"}</span>
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="space-y-4 pr-1">
                                                ...
                                                <div className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-2xl text-xs">
                                                    <div>
                                                        ...
                                                        <span className="text-[9px] text-foreground/45 block mt-0.5">Toggle light or dark modes</span>
                                                    </div>
                                                    <div className="flex bg-zinc-900 border border-white/5 p-1 rounded-xl select-none">
                                                        <button
                                                            onClick={() => updatePrefTheme("dark")}
                                                ...
                                                <div className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-2xl text-xs">
                                                    <div>
                                                        <span className="font-semibold text-white block">Player Subtitle Size</span>
                                                        <span className="text-[9px] text-foreground/45 block mt-0.5">Changes size inside player subtitles</span>
                                                    </div>
                                                    <select
                                                        value={prefSubtitleSize}
                                                        onChange={(e) => updatePrefSubtitleSize(e.target.value)}
                                                    ...
                                                    >
                                                        ...
                                                        <option value="22px">Medium (22px)</option>
                                                        <option value="26px">Large (26px)</option>
                                                ...
                                                <div className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-2xl text-xs">
                                                    <div>
                                                        ...
                                                        <span className="text-[9px] text-foreground/45 block mt-0.5">Automatically plays next checklist entry</span>
                                                    </div>
                                                    <button
                                                        onClick={() => updatePrefAutoplay(!prefAutoplay)}
                                                        className={`w-10 h-6 flex items-center rounded-full p-1 transition-colors duration-300 cursor-pointer ${
                                                            prefAutoplay ? "bg-primary" : "bg-zinc-800"
                                                        }`}
                                                    >
                                                        <div
                                                            className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-300 ${
                                                                prefAutoplay ? "translate-x-4" : "translate-x-0"
                                                            }`}
                                                        />
                                                    </button>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between p-3.5 bg-white/5 border border-white/5 rounded-2xl text-xs">
                                                    <div>
                                                        <span className="font-semibold text-white block">Auto Resume Video</span>
                                                        <span className="text-[9px] text-foreground/45 block mt-0.5">Start video from last saved progress timestamp</span>
                                                    </div>
                                                    <button
                                                        onClick={() => updatePrefAutoResume(!prefAutoResume)}
                                                        className={`w-10 h-6 flex items-center rounded-full p-1 transition-colors duration-300 cursor-pointer ${
                                                            prefAutoResume ? "bg-primary" : "bg-zinc-800"
                                                        }`}
                                                    >
                                                        <div
                                                            className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-300 ${
                                                                prefAutoResume ? "translate-x-4" : "translate-x-0"
                                                            }`}
                                                        />
                                                    </button>
                                                </div>
