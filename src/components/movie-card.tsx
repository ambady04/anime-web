...
export default function MovieCard({ subject }: MovieCardProps) {
  ...
  return (
    <Link href={watchLink} className="group relative block w-full aspect-[2/3] rounded-2xl overflow-hidden glass-card transition-all duration-300">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/35 to-transparent opacity-85 sm:opacity-65 group-hover:opacity-95 transition-opacity duration-300 flex flex-col justify-end p-4 z-10">
        ...
        <div className="flex items-center justify-between mb-1.5 flex-wrap gap-1">
          ...
          {subject.imdbRatingValue > 0 && (
            <div className="flex items-center space-x-1 bg-black/60 backdrop-blur-md px-2 py-0.5 rounded-md border border-white/10">
              ...
              <span className="text-[10px] font-black text-yellow-500">{subject.imdbRatingValue.toFixed(1)}</span>
            </div>
          )}
