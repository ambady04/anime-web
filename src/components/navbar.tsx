...
import { Search, History, Heart, Home, Film, User, Sun, Moon } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <header className="sticky-nav py-3.5 transition-all duration-300">
      <div className="max-w-[95rem] mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
        {/* Logo Section */}
        <Link href="/" className="flex items-center space-x-2 group relative z-10">
          ...
          <div className="flex-shrink-0 relative w-6 h-6 flex items-center justify-center">
            <svg className="w-5 h-5 filter drop-shadow-[0_0_2px_var(--primary-glow)]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L5 9L12 16L19 9L12 2Z" fill="#E31C25" fillOpacity="0.9" />
              <path d="M12 8L5 15L12 22L19 15L12 8Z" fill="#1A1A1A" fillOpacity="0.85" className="dark:fill-black" />
            </svg>
          </div>
          <div className="flex flex-col">
            <div className="flex items-center text-xl font-black tracking-wider leading-none">
              <span className="text-foreground transition-colors duration-300">KI</span>
              <span className="text-primary transition-colors duration-300">XO</span>
            </div>
